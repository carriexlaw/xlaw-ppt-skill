...
- 序号不应该有强烈的存在感
  - 大圆形容器 + 小数字不再用。两种处理：缩小圆形容器（直径 ≈ 1.5–1.7 × 旁边文字的字号）+ 数字加粗；或去掉容器
  - 去掉容器的写法：借目录页的数字样式（英文字体细体、`data_muted`、去掉前导 0、字号 48–54），数字后加一条同色细竖线（1.5pt，高 = 要点文字高），再接要点文字；要点文字用细体，关键词用 Medium（类似小标题，不用 Bold）+ accent
  - 相邻但不属于同一个 series 的两页，序号 / 容器 / icon 这类视觉元素不要一样：上一页用序号圆，下一页就换目录式数字 + 细线
- 要点之间的间距 ≤ 要点模块到大标题、到页面下边缘的距离：并列要点之间用 2g，不用 3g（3g 是标题区到 body 的距离）；剩余空间留在模块之外，整块居中
