...
### 自查结论文件 `_qa/review.md`

自查由看图的一方写成文件，交付与改页都只读这个文件。格式：

```
## deck
Flip: ... / Silhouette: ... / Anchor: ...
## pages
05 | 一秒看到：三个方案名（= focus.object）| 第二大：结论句 24 | message 关键词：产能 32 / 成本 18 | 可删：无 | 通过
12 | 一秒看到：大数字 48%（focus.object 是「三类客户」）| … | 不通过：数字抢了论述对象
## fixes
12：大数字降到 number 下限，客户类型升 heading
```
